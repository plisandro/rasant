...
impl Map {
	...
	/// Copies all contents from another [`Map`] into this one.
	...
	/// Returns the number of attributes for the [`Map`].
	pub fn len(&self) -> usize {
		self.keys.len()
	}

	/// Evaluates whether the [`Map`] has no attributes defined.
	...
	/// Returns an attribute (key, [`Value`], [`Metadata`]) iterator for all attributes in the [`Map`].
	...
	/// Evaluates whether a given attribute is present in the [`Map`].
	...
	/// Returns an attribute [`Value`] + [`Metadata`] in the [`Map`] for a given key.
	...
	/// Returns an attribute [`Value`] in the [`Map`] for a given key.
	...
	/// Inserts a [`Value`] into the [`Map`], by reference.
	...
	/// Inserts an ephemeral [`Value`] into the [`Map`], by reference.
	pub fn insert_ref_ephemeral(&mut self, key: &str, val: &Value) {
		self.set(key, val, true);
	}

	/// Inserts an ephemeral [`Value`] into the [`Map`].
