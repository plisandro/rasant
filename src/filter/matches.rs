...
impl Filter for AttributeKey {
	...
	fn pass<'f>(&mut self, update: &'f sink::LogUpdate) -> bool {
		...
		for (key, _, _) in update.attribute_iter() {
...
impl Filter for AttributeValue {
	...
	fn pass<'f>(&mut self, update: &'f sink::LogUpdate) -> bool {
		let Some((val, _)) = update.attribute_get(self.key.as_str()) else {
			return false;
		};
		...
		for i in 0..cmp::max(self.has.len(), self.has_not.len()) {
			...
			for s in ss {
				s.into_raw_string(&mut self.str_cache, update);
...
mod tests {
	...
	fn attribute_keys_single() {
		fn run(mut filter: AttributeKey, want: bool) {
			let mut fixed = attributes::Map::new();
			fixed.insert("a_string", Value::from("hello there!"));
			fixed.insert("an_int", Value::from(12345));
			fixed.insert("a_float", Value::from(6789.0123 as f32));

			let update = sink::LogUpdate::from((Timestamp::now(), Level::Info, 0, "unused update :(", &fixed));
			assert_eq!(filter.pass(&update), want);
		}
